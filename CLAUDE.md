...
Sample audio for local/CLI testing is not checked into the repo (see `.gitignore` — `ci_music/.gitignore` excludes `src/input/` and `src/output/`). Use your own WAV/MP3 files for manual testing. This is separate from `web/samples/` — two short public-domain clips bundled into the GitHub Pages build so visitors without an audio file on hand can still try the live demo (see `web/samples/README.md` for sourcing/licensing).

**Live playback visualization** (`web/index.html` + `web/viz.js`): while audio plays, the cochlea spiral and frequency bar light up per-band using a Web Audio `AnalyserNode`. `ANALYSER_FFT_SIZE` (in `viz.js`) must stay large enough that adjacent low-end log-spaced bands resolve to mostly-distinct FFT bins — a smaller fftSize makes many low bands collapse onto the same one or two bins and light up together as a single block regardless of actual content. `litThreshold()` sets the per-frame lit cutoff relative to that frame's loudest bin rather than a fixed dB floor, for the same reason. Both are covered by `viz.test.js`, including a regression test for the bin-collision case.

**Theming** (`web/index.html`): light/dark is driven entirely by CSS custom properties defined in `:root` (dark, the default) with a light palette applied two ways — `@media (prefers-color-scheme: light) { :root:not([data-theme="dark"]) {...} }` for OS-level auto-detection, and an unconditional `:root[data-theme="light"] {...}` block for the manual toggle (top-right button) to force either theme regardless of system preference. The choice is persisted to `localStorage` (`koru-theme`) and applied via a synchronous inline script in `<head>` before first paint, to avoid a flash of the wrong theme. The few colors the JS draws directly into the SVG (spiral track, unreachable-channel gray, modiolus cap fill) are set as `'var(--x)'` strings via `.style`/`element.style.fill`, not resolved hex — so they follow the live CSS cascade automatically on toggle, with no rebuild/refresh logic needed in JS.
